...
def wait_for_nginx(base_url: str, timeout_seconds: float = 5.0) -> None:
...
@pytest.mark.ui
@pytest.mark.slow
def test_nginx_form_submit_example_5():
    browser = os.getenv("BROWSER", "chrome")
    headless = os.getenv("HEADLESS", "false").lower() == "true"
    print(f"Uzywam przegladarki: {browser}, headless={headless}")

    static_dir = Path(__file__).resolve().parent / "nginx_form"
    ...
    with (
        ...
        try:
            ...
            time.sleep(10)
            print("Formularz wyslany i wynik widoczny")
        finally:
            driver.quit()
